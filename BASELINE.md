...
As of 2026-06-13 our `fa2sw_precache_kenyan` split-KV launch (**PR #52**) holds **public #1 at 481.53 official TPS** (PPL 2.3772, 128/128, all modalities). The ladder we reproduced then beat:
...
- **Open:** our #1 is on the **public** set. The private re-run gate (the programme's #1 risk, see "Key risk" below) is **not yet cleared** for this stack — leaderboard row also pending organizer re-sync.
...
- **OFFICIAL FRONTIER (a10g-small HF-Job confirmed — NEW PUBLIC #1) — `submissions/fa2sw_precache_kenyan` (PR #52 launch, lawine; stack from #43) — official a10g-small tps=481.53, ppl=2.3772, 128/128 VALID, all modalities loaded** (job
  `6a2dce05871c005b5352c0b9`, run `results/senpai/fa2sw-precache-kenyan-20260613T213911Z`, W&B `2x9fm2zx`/`fwo8rs05`). Linear MTP K=7 (M=8 verify) + 3D split-KV. **New public #1** vs rock-ai 459.72 (+4.74%); **+13.4%** over the ~424.5 frontier-repro baseline. **The official bar all submissions must beat is now 481.53 TPS.** Standing open axis: the **private re-run gate** (kanna #44 probe predicts ~12.4% on a pure-chat proxy; kanna #55 calibrating) — public #1 confirmed, private stability not yet certified.
...
- **Standing risk (UNCHANGED, the programme's #1):** the **private re-run gate**. kanna #44 probe predicts ~12.4% public→private on a pure-chat proxy (WOULD-FAIL >5%); kanna #55 is calibrating the probe on this exact frontier. The 481.53 is the **public** official number; private stability is a separate, open axis.
