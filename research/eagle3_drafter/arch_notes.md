...
## 9. PR #34 — benchmark-matched reasoning corpus (breaking the 0.73 plateau)

**PR:** #34 · **Author:** fern · **Date:** 2026-06-14
**Hypothesis:** the #25 reasoning-acceptance plateau (~0.73 tf_acc) is a *prompt-template
+ distribution* mismatch, not a capacity/step-count limit. Training on greedy CoT
self-distilled from the served target under the **exact** benchmark prompt templates
breaks the plateau. Offline only — own GPU, no HF Job.

### 9.1 Corpus (`gen_eagle3_corpus.py build_reasoning_corpus`)
Three streams, each rendered into the byte-matched benchmark template from
`eval_prompts_sharegpt.json`, assistant CoT generated by **greedy** decode from
`google/gemma-4-E4B-it` (the served target), `max_new_tokens=1024`, concurrency 16:
- **mmlu_pro** — `TIGER-Lab/MMLU-Pro` (test+validation), **non-science** categories,
  A–J MCQ template (`render_mmlu_pro`).
- **gpqa** — real GPQA is HF-gated on this pod, so the PR-sanctioned **A–D proxy**:
  `TIGER-Lab/MMLU-Pro` **physics/chemistry/biology** re-rendered A–D (`render_gpqa`,
  `iter_mmlu_pro(sci_only=True)`). Disjoint from the mmlu_pro stream (`seen_q`).
- **aime** — `AI-MO/NuminaMath-CoT` competition subsets (amc_aime/aops_forum/
  olympiads/synthetic_amc) in the AIME `ANSWER: $ANSWER` template (`render_aime`).

Mix ≈ benchmark ratio **0.445 / 0.445 / 0.11** by token volume. **De-contamination
(hard, fail-loud):** `load_eval_decontam` loads all 128 official eval prompts (raises
if missing); `_accept_prompt` drops any train question whose normalized body OR
rendered prompt matches an eval prompt; plus the PR #16 token-id backstop (dropped
0/3023). Result: **3023 train records / 2.63M tok** (mmlu_pro 1279/1.17M, gpqa
1424/1.17M, aime 320/0.30M) and a disjoint **240-record / 159k-tok holdout**
(mmlu_pro 107, gpqa 107, aime 26), saved with per-source `source` tags + per-source
holdout shards.

### 9.2 Training (`train_eagle3.py`, run `56ksyxgw`)
Warm-start from #25 `full_20k/model_best.pt` (15/15 tensors, 0 missing/unexpected),
**lr 5e-5** (½ of #25's 1e-4), cosine, warmup 150, eval+ckpt every 250 steps,
best-select by **held-out reasoning tf_acc** (not val loss). Same arch (aux `[2,21,39]`,
`feature_shift=1`). **4500 steps / 48.3 min / peak 13.58 GB.** Held-out tf_acc rose
monotonically 0.657 (step 250) → **0.7617** (step 4500, best==final; no tf overfit
even as train acc → 0.99).

### 9.3 Results — apples-to-apples on the 240-record benchmark-matched holdout
Both checkpoints evaluated with `eval_eagle3.py --native` on the **same** holdout
(`feature_shift=1`). Native = free-running EAGLE chain (K=8); `native_step1_top1`
is a wiring self-check that must ≈ tf top-1.

| metric | #25 `full_20k` (`cjhjnsff`) | new bench (`gua9x68j`) | Δ |
|---|---|---|---|
| **tf_acc top-1** | 0.4709 | **0.7617** | **+0.291** |
| tf top-4 | 0.6400 | 0.8903 | +0.250 |
| **native accept/step (K=8)** | 0.4315 | **0.7792** | **+0.348** |
| native step-1 top-1 (≈tf check) | 0.4260 | 0.7714 | — |
| loss | 3.2286 | 1.1702 | −2.06 |
| tf aime / gpqa / mmlu_pro | 0.692 / 0.484 / 0.400 | 0.843 / 0.803 / 0.701 | +0.15 / +0.32 / +0.30 |
| native aime / gpqa / mmlu_pro | 0.663 / 0.440 / 0.367 | 0.827 / 0.804 / 0.742 | +0.16 / +0.36 / +0.38 |

**Findings.**
1. **The 0.73 plateau was a distribution/template artifact.** #25 (trained on raw
   MATH+ShareGPT) collapses to **0.47 tf** on the benchmark distribution — far below
   its 0.7314 MATH-holdout number. The matched corpus recovers to **0.76**. Per-source
   ordering proves the mechanism: **aime** (closest to #25's free-form MATH) has the
   smallest gap; the **MCQ templates #25 never saw** (mmlu_pro, gpqa) have the largest.
2. **The lift converts to native** (+0.348 accept/step, +81% rel). It converts because
   this stack's **step-0 is serve-faithful** (`feature_shift=1`; native step-1 ≈ tf,
   |Δ|≤0.045 for both ckpts), so the tf gain flows straight into native step-1, which
   dominates accept/step. This is the *opposite* of the #9 tf↔native anti-correlation,
   whose root cause (train on `f_j`, serve on `f_{j-1}`) is **absent** here.
3. **Residual native ceiling = the K=1 training regime, not the corpus.** Both drafters'
   chains nearly collapse past step 1 (accept/step ≈ step-1 hit), because neither was
   trained on its *own* hidden states (no multi-step/TTT unroll). This is the shared
   limitation the next PR targets (HASS-style TTT / Draft-OPD on-policy distillation),
   **not** corpus quality and **not** step-0.

**Caveat:** the gpqa stream is the A–D MMLU-Pro science proxy (real GPQA gated). A true
serve-side L2/acceptance probe needs the vLLM stack (gated on verify-rollback #24);
the native sim here is the offline serve-faithful proxy.
