...
1. ``STARK_PIN_MODE=fixed2d`` -> set
   ``vllm.v1.attention.backends.triton_attn.MIN_LAUNCH_GRID_SIZE_2D = 0`` at module
   import. With the threshold 0, the M=1 AR decode forward (num_seqs=1 > 0) selects
   the 2D single-pass kernel instead of the default 3D split-KV kernel, matching the
   M>=2 verify forward (land #684's fixed-split pin). NOT a kernel rebuild.
...
def _wrap_unified_attention(module) -> None:
    """Record import-time is_batch_invariant and wrap unified_attention to log the
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            a = bound.arguments
            num_seqs = len(a["seqused_k"])
            max_seqlen_q = int(a["max_seqlen_q"])
            st3 = a.get("seq_threshold_3D")
            nps = a.get("num_par_softmax_segments")
            so = a.get("softmax_segm_output")
            sm = a.get("softmax_segm_max")
            se = a.get("softmax_segm_expsum")
            ibi = bool(getattr(module, "is_batch_invariant", False))
            # Exact replica of triton_unified_attention.unified_attention's
            # 2D-vs-3D selection (vllm 0.22.0, lines 923-932): use_3d True ->
            # 3D split-KV; False -> 2D single-pass.
            use_3d = not (
                st3 is None
                or nps is None
                or so is None
            ...
            if shape not in _seen_shapes and len(_seen_shapes) < _MAX_SHAPE_ROWS:
