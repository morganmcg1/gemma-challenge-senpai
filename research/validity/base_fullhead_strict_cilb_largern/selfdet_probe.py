#!/usr/bin/env python3
"""PR #564 — self-determinism probe for the base_fullhead serve. LOCAL, NO FIRE.

The load-bearing arm of this card is base_fullhead (the submission's surgical 2D
order-preserving attention + the PLE embed-scale fold). #557 already probed the
vanilla ple_fold serve (self_det=false at completion level, answer-level 22/24);
this probe records the SAME self-consistency profile for the surgical-attention
serve so the program has a base_fullhead-specific number.

Brings up ONE base_fullhead server and runs the SAME small byte-identical
MMLU-Pro subset through the greedy client TWICE, then compares the full completion
text per item (from the two inspect .eval logs) AND the scored answers.
self_det=true iff every item's completion is byte-identical across the two passes.
...
just records whether the serve is self-consistent. One model load, two quick
passes, then teardown. Writes selfdet.json (read by aggregate.py).
"""
...
def main() -> int:
    run_arms.wait_gpu_free()
    log = HERE / "server_selfdet.log"
    proc = run_arms.start_server(ARM, log)
    out = {"arm": ARM, "n_probe": N_PROBE,
           "build": "vllm-0.22.1rc1.dev307+g3e8afdf78"}
    try:
        run_arms.wait_ready(proc)
        print("[selfdet] base_fullhead server READY — running two greedy passes", flush=True)
        outs = []
        for tag in ("a", "b"):
            o = HERE / f"selfdet_pass_{tag}.json"
            cmd = [
                str(EVAL_PY), str(QE / "run_eval.py"), "--task", "mmlu_pro",
                "--arm", f"selfdet_{tag}", "--out", str(o), "--seed", str(run_arms.SEED),
                "--n", "500", "--limit", str(N_PROBE), "--max-tokens", "2048",
                "--max-connections", "32", "--base-url", f"http://127.0.0.1:{run_arms.PORT}/v1",
    ...
    print(f"[selfdet] arm={ARM} self_det={out.get('self_det')} "
