...
## 2026-06-15 02:33 — PR #276: Acceptance local→official transfer (τ_acc) — 🟢 `tau_acc=1.0`, λ̂ is a KERNEL-INVARIANT proxy (no τ correction); safe local bar 0.985548 — bank-the-analysis (0 TPS)

- **Branch:** `lawine/acceptance-local-official-transfer` · **Student:** lawine · merged 02:33Z by morganmcg1 (advisor; LOCAL re-serve λ̂ measurement at M∈{1,8,16}, greedy, 32,640 positions, no served-file change; BASELINE 481.53, 0 TPS). W&B [`vcgtsl1c`](https://wandb.ai/wandb-applied-ai-team/gemma-challenge-senpai/runs/vcgtsl1c) (group `acceptance-local-official-transfer`, finished, NaN-clean, all 7 sub-gates) — Morgan W&B-verified (`tau_acc=1.0`, composition round-trip `K_cal(125.268)×E_T(3.844)=481.53` resid 0).
- **Primary:** `acceptance_local_official_transfer_self_test_passes=1`. **Test:** `tau_acc=1.0` (the exact opposite of the TPS leg's `τ_lo=1.0352`).
- **Key finding — λ̂ transfers 1:1, kernel-INVARIANT (unlike TPS):** λ̂ is a clock-invariant probability (per-token argmax agreement), so official λ̂ == local λ̂ at matched batch width up to a small SYMMETRIC kernel-jitter ~0.73%. Per-M λ̂ table: M1↔M8 (served linear) **0.992708** (div 0.007292), M8↔M16 (linear→tree) 0.992800 (div 0.007200), M1↔M16 (tree) 0.992463 (div 0.007537); determinism + within-batch controls = **1.000000 at every M** (zero harness noise). The M8↔M1 divergence **reproduces lawine #232 (`nxwv6pam`) = 0.007292 EXACTLY** (6 decimals — the slice that fed fern #249's bar). Divergence **saturates after M≥8** (8↔16 < 1↔8 < 1↔16): fixed near-tie flip rate, not a runaway batch effect. **Architectural clarification:** the int4-Marlin body GEMMs are **bit-exact across M=1/8/16** (`max_abs_diff=0.0`) — ZERO argmax batch-variance; the entire ~0.73% jitter lives in the **bf16 lm_head + TRITON_ATTN** accumulation (the quantized core is fully batch-invariant — good news for verify-side kernel retunes).
- **Conclusion — closes the ACCEPTANCE leg of the launch-gate transfer (2/3 legs now closed):** read local λ̂ DIRECTLY against the 0.9780 bar with no τ_acc correction (unlike τ_lo); safe local build bar = **0.985548** (build to local λ̂ ≥ 0.9855 to clear official 0.9780 under worst-case ±0.0075 jitter). With τ_lo (TPS #267) this gives two of the three official-gate transfer legs; the PPL leg (τ_ppl) + the composite predictor remained open → originally the next lawine reseat, now folded into the surviving E[T]-axis work (lawine #282 measures the per-prompt E[T] distribution; the bf16-lm_head jitter locus is the greedy-safety margin for any verify-side retune). 

## 2026-06-15 02:22 — PR #277: Draft io_projection roofline — ⚪ NULL, intrinsic-M=1 (BW-util 46.7%); CLOSES the draft-pass decomposition, `projected_tps_gain_pct=0.0` — bank-the-analysis (0 TPS)

- **Branch:** `kanna/draft-io-projection-roofline` · **Student:** kanna · merged 02:22:58Z by advisor (senpai; LOCAL GPU micro-profiling + CPU analytic, no served-file change; BASELINE 481.53, 0 TPS). W&B [`ahw089yi`](https://wandb.ai/wandb-applied-ai-team/gemma-challenge-senpai/runs/ahw089yi) (group `draft-io-projection-roofline`, finished, NaN-clean, self-test 6/6 a–f) — advisor W&B-verified (`projected_tps_gain_pct=0.0`, `io_projection_measured_us=14.044`, `io_bw_utilization=0.4666`, `io_overhead_ratio=2.143`, `recoverable_material=False`, `frontier_tps=481.53` unchanged).
- **Primary:** `draft_io_projection_roofline_self_test_passes=1`. **Test:** `projected_tps_gain_pct=0.0`.
- **Key finding — io_projection is the 2 residual-stream GEMVs, NOT q/k/v/o:** reading #264's `build_gemv_buckets`, the "io_projection" bucket = `pre_projection [256,5120]` (backbone→draft, pass start) + `post_projection [2560,256]` (draft→backbone, pass end) — the q/k/v/o are #264's SEPARATE attention bucket (wirbel #270). The PR's "many small q/k/v/o launches per layer" premise was a misrecollection: io_projection is 2 pure GEMVs at opposite ends of the 4-layer stack, NO companion kernel, NON-adjacent (no fuseable pair). Measured 14.04µs recovers #264's 13.86µs (1.3%); `io_bw_utilization=46.7%` (vs 80.9% on the saturating 128 MiB anchor GEMV) ⇒ intrinsic-M=1 memory-latency-bound, exactly like the MLP GEMVs. `recoverable_io_us=0.06µs → recoverable_material=False → 0.0%`. Unlike the MLP (+4.39% GeluAndMul fold), io has NO separable-launch slack.
- **Conclusion — CLOSES the draft-pass decomposition:** MLP (#269) + attn (wirbel #270) + io (this) = 93.27µs = **95.2% of the 98µs GEMV chain, all intrinsic-M=1**. The ONLY recoverable draft slack anywhere is the MLP's +4.39% GeluAndMul fold; the bf16 draft floor is essentially all intrinsic-M=1 — the only draft levers are fewer passes (adaptive-K, ONEGRAPH-blocked) or M≥16 batching (Path-A axis). Reseat kanna #280 (advisor) → **verify-step component roofline** (the 88%-of-step `verify_us(M)`, never decomposed — apply the same BW-util diagnostic + price wirbel #270's num_stages=2 SDPA lever + φ-correct).

## 2026-06-15 02:20 — PR #274: Composition-overhead honesty (φ) — 🟢 DECISIVE, draft-CUT axis DEAD; honest 500 needs E[T]_real ≥ 3.991, `honest_corrected_static_k4_tps=493.96` — bank-the-analysis (0 TPS)

- **Branch:** `fern/composition-overhead-honesty` · **Student:** fern · merged 02:20:13Z by advisor (senpai; CPU-analytic φ decomposition over the served anchor, no GPU/served-file change; BASELINE 481.53, 0 TPS). W&B [`brnmnl60`](https://wandb.ai/wandb-applied-ai-team/gemma-challenge-senpai/runs/brnmnl60) (group `composition-overhead-honesty`, finished, NaN-clean, self-test pass) — advisor W&B-verified (`composition_overhead_honesty_self_test_passes=1`, `honest_corrected_static_k4_tps=493.957`, `phi_measured=0.6035`, `phi_assumed=0.1253`, `any_draft_lever_clears_500_assumed=0`, `honest_et_real_floor=3.9914`, `honest_tree_width_measured_gd=539.50`).
- **Primary:** `composition_overhead_honesty_self_test_passes=1`. **Test:** `honest_corrected_static_k4_tps=493.957` (< 500).
- **Key finding — the φ-correction erases every draft-CUT "clears 500" claim:** the real wall step = `fixed_overhead + draft_cost(K) + verify_us(M)`; solving at the served anchor gives φ = model_forward_fraction ∈ **[0.125 (assumed g_d 0.1253), 0.735 (measured g_d 0.6035)]**, and the realization ratio (honest_gain/comp_gain) = φ. So stark #266's static-K=4 composition 502.12 → **honest 493.96 / 484.11** (both φ edges, < 500); the rigorous wall-step recompute makes static-K=4 a **−17% LOSS** (397–401 TPS) — cutting K barely shrinks the step but drops E[T] 3.844→3.080. **Lever-agnostic floor: honest 500 needs E[T]_real ≥ 3.991 (+3.8%), and draft-cuts move AWAY from it.** ~40% of the wall step is FIXED serving overhead (~3.9 ms), un-attacked by any draft/verify step-shaving. fern's tree-width "survivor" (honest 539.5) used denken #268's WRONG assumed-g_d 577.6 base → SUPERSEDED by denken #271 (measured g_d → 479.6 NO-GO).
- **Conclusion — "you cannot CUT your way to 500; only E[T]-RAISING works":** combined with denken #271 (width NO-GO 479.6), BOTH the draft-cut axis AND the width axis fail to reach 500. The only surviving path is E[T]-raising to E[T]_real ≥ 3.991, bounded by ubel #263 (private E[T] degrades to 73% OOD). Reseat fern #281 (advisor) → **E[T]-raise feasibility envelope** (the three-axis Path-A closure verdict: does ANY achievable E[T]_real reach the step-shave-lowered floor, or is Path-A fully closed?).

## 2026-06-15 02:10 — PR #270: Draft/tree attention Triton autotune — ⚪ draft-attn NEGLIGIBLE; VERIFY-SDPA MISTUNED (num_stages 3→2 bit-identical ~1.09×), `tps_bitident=487.70` — bank-the-analysis (0 TPS)

- **Branch:** `wirbel/draft-tree-attention-autotune` · **Student:** wirbel · merged 02:10:50Z by advisor (senpai; LOCAL Triton autotune over the served-wheel attention, bit-identity gated, no served-file change; BASELINE 481.53, 0 TPS). W&B [`iwwcmvez`](https://wandb.ai/wandb-applied-ai-team/gemma-challenge-senpai/runs/iwwcmvez) (group `draft-tree-attention-autotune`, finished, NaN-clean, self-test pass) — advisor W&B-verified.
- **Primary:** `draft_attn_triton_autotune_self_test_passes=1` · `triton_attn_autotune_greedy_safe=1` · `greedy_identical_by_construction=1`. **Test:** `projected_tps_gain_pct=1.2813` (draft upper-bound) / `tps_bitident=487.70` (+6.17, the verify-SDPA lever).
- **Key finding — the DRAFT attention is a NULL, but the VERIFY SDPA is mistuned:** `draft_tag=NEGLIGIBLE` — the in-graph (ONEGRAPH) draft attention residual is ~null; the best bit-identical draft autotune saves only ~2.2µs/pass = **+1.28% upper bound** (kanna-residual lower bound +0.056%), intrinsic-M=1 like the MLP/io GEMVs. **The real find is on the VERIFY side:** the head-512 verify SDPA ships triton-default `num_stages=3`, and `num_stages=2` is **bit-identically ~1.09×/call** (`verify_speedup_bitident` = 1.0968 / 1.0899 / 1.0924 at M=8/16/32, validated against the live wrapper) ⇒ `verify_mistuned=1`, `tps_bitident=487.70` (+6.17 composition). `any_tile_diverged=1` (some sweep configs FP-reassociate and were excluded; the credited num_stages=2 is bit-identical, greedy-safe by construction).
- **Conclusion — first greedy-safe VERIFY-side step lever:** the draft attention joins the MLP/io as intrinsic-M=1 (no draft-side slack), but the verify SDPA carries real bit-identical recoverable slack (~1.09×/call). The per-call-ratio→precise-TPS conversion (via the SDPA share of `verify_us(M)`) was handed to denken #257 / kanna's verify-step component roofline (#280). Reseat wirbel #279 (Morgan) → **verify-SDPA linear-deploy transfer** (does the num_stages=2 tune transfer to deployed batch=8 in wall-clock?).

## 2026-06-15 02:03 — PR #271: Deployed-path g_d reconciliation — 🟢 DECISIVE, g_d fork COLLAPSES to NO-GO; `deployed_gd=0.01906`, M*=32 → 479.57 < 500 — bank-the-analysis (0 TPS)

- **Branch:** `denken/deployed-gd-reconciliation` · **Student:** denken · merged 02:03:14Z by advisor (senpai; LOCAL coupled-region CUDA-graph profiling on the ONEGRAPH-faithful basis, no served-file change; BASELINE 481.53, 0 TPS). W&B [`9mlmaen3`](https://wandb.ai/wandb-applied-ai-team/gemma-challenge-senpai/runs/9mlmaen3) (group `deployed-gd-reconciliation`, finished, NaN-clean, self-test 5/5 a–e) — advisor W&B-verified.
- **Primary:** `gd_step_basis_reconcile_self_test_passes=1` · `gd_fork_decisive=1` · `anchor_is_gd_invariant=1`. **Test:** `deployed_gd=0.019060` (CI [0.01905, 0.01907]) / `tps_at_Mstar32_deployed=479.571`.
- **Key finding — the deployed-path g_d collapses denken #268's go/no-go fork to NO-GO:** measuring the draft K=7 → verify(8) as back-to-back CUDA graphs on one stream (the ONEGRAPH-faithful deployed basis, NOT eager) gives `deployed_gd=0.01906`, ~3% from #257's isolated 0.01903, with **zero coupling overhead** (×0.9998). The 500-TPS pivot is `g_d*=0.04018`; deployed 0.0191 is **≪ pivot** (`margin_gd_to_pivot=−0.0211`, `deployed_gd_above_pivot=0`). An HBM read-floor INDEPENDENTLY caps any g_d at **0.0348** (a g_d above pivot would need verify(8) ~4.8× faster than the int4 read floor — physically impossible). At M*=32 (the #268 λ-valid ceiling) the deployed path prices **479.57** (`tps_gap_to_500=−20.43`, `width_clears_500_deployed=0`); all E[T] corners miss (rank-cov 5.157→479.6, #257-flat 4.512→419.6, meas-floor 4.330→402.7). The eager-draft g_d=0.0747 (above pivot) is architecturally EXCLUDED because the deployed manifest sets ONEGRAPH=1.
- **Conclusion — tree WIDTH alone cannot reach 500 on the deployed path:** denken #268's central-NO-GO (480) vs optimistic-GO (577.6) fork is RESOLVED to the central NO-GO basis. Step model `step(M;g_d)=step_served·[verify_us(M)/v8 + n_tree·g_d]/(1+K_spec·g_d)` (step_served=1218.2µs, v8=5163.71, n_tree=5, K_spec=7). Combined with fern #274 (draft-cut axis dead), this leaves ONLY the bounded E[T]-raise axis. Reseat denken #278 (Morgan) → **deployed linear-step decomposition** (M=1 verify + fixed overhead for the φ denominator).
