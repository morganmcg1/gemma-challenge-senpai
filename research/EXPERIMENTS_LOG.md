...
## 2026-06-15 04:40 — PR #291: Verify-compute hideability — 🟢 honest kernel-addressable floor 487.7 TPS (only 4.8% of verify-compute hides); free step lane to 500 does NOT exist; step-side DEFINITIVELY CLOSED — bank-the-analysis (0 TPS)

- **Branch:** `denken/verify-compute-hideability` · **Student:** denken · merged 04:40:10Z by advisor (senpai; CPU-analytic + reused/fresh CUDA-event grounding, no model load, no served-file change; BASELINE 481.53, 0 TPS). W&B [`3myn1fzl`](https://wandb.ai/wandb-applied-ai-team/gemma-challenge-senpai/runs/3myn1fzl) (canonical, fresh GPU probe) + [`myttnvah`](https://wandb.ai/wandb-applied-ai-team/gemma-challenge-senpai/runs/myttnvah) (analytic replicate, bit-identical) — group `verify-compute-hideability`, finished, NaN-clean, self-test 7/7. Independently W&B-verified by advisor (all 6 checks MATCH bit-for-bit across BOTH runs: self_test=1, tps_kernel_floor_honest=487.7289, verify_compute_hideable_frac=0.0482, tps_all_compute_hides=746.91, free_lane_to_500_exists=0, nan_clean=1). Merge commit `14d6a268`.
- **Primary:** `verify_compute_hideability_self_test_passes=1` (7/7). **Test:** `tps_kernel_floor_honest=487.7289`.
- **Key finding:** of denken #283's 2104.6µs verify-above-read compute, only **4.82% (0.0482) is overlap-hideable greedy-safe** (the SDPA num_stages lever) — landing the honest kernel-addressable floor at **487.7289 TPS**, inside [487.7, 746.9]. This REPLACES #283's optimistic all-hides ceiling 746.9 (which assumed 100% of verify-compute hides) with the MEASURED-fraction floor: `free_lane_to_500_exists=0`, `extra_hideable_needed_for_500=193.43µs` (1.906× the realized lever). The 487.7 floor coincides EXACTLY with wirbel #285's free-lossless ceiling (487.729) — two independent derivations of the same free step-side ceiling. The exposed ~2003µs residual decomposes (diagnostic) into irreducible non-body memory + greedy-unsafe MLP compute — touchable only by a PPL/λ-gated math-changing kernel retune (belongs in a measured build, not a free lane).
- **Conclusion — the step-side is DEFINITIVELY CLOSED:** no free (zero-cost, greedy-safe) step lane reaches 500 — the measured kernel-addressable floor is 487.7 < 500. Confirms denken #283's reframe (the verify front was the last open step-side hope; now measured-closed) and seals the wirbel #285/#286/kanna #286 487.7/493.6 envelope. The sole >500 path is the human-gated E[T]-raise BUILD (numerator), NOT any step-side (denominator) lever. ubel #284 owns the orthogonal host/other 2109µs (26%) block. Reseated denken #297 (advisor) → tail-resolved per-position (the build-numerator complement: does the hard-prompt acceptance cliff shift?).
