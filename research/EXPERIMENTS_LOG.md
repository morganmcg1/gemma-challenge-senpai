...
## 2026-06-14 16:46 — PR #176: Adverse domain-skew private stress (descent-only survival at 500) — 🟢 GREEN / descent-only holds 504.15 at the adverse τ-low corner — MERGED (bank-the-analysis)

- **Branch:** `stark/descent-private-adverse-skew` · **Student:** stark · merged 16:46:07Z by morganmcg1 (CPU-only analytic stress — no HF Job/submission/served-file change; BASELINE unchanged 481.53, adds 0 TPS; greedy/PPL untouched, no launch authorized). W&B `uzl7ixll` (finished, NaN-clean). Advisor-verified: self-test passes.
- **Primary:** `adverse_skew_stress_self_test_passes=1`. **Test:** `descent_only_taulow_tps_adverse_corner=504.15`.
- **Key finding:** under an adverse private-domain skew (worst-case private-vs-served distribution gap), descent-only still clears 500 at the conservative τ-low corner (**504.15 TPS**). The private skew does NOT subtract enough to flip the descent-only verdict at the adverse corner — the served→private gap (stark's axis) is not a launch-binding term once both-bugs stays the robust shot. Also shipped a non-fatal robustness fix to `scripts/validity/private_gap_probe.py` (wrapped `log_report_to_wandb` in try/except — the submission venv's wandb was missing `wandb.init`; logging now degrades gracefully instead of crashing the probe).
- **Conclusion:** retires the private-skew axis as a launch blocker for the robust (both-bugs) path — even the adverse corner clears 500. Pairs with fern #179's packet (the GO this stress-tests) + land #71 (whose measured λ remains the sole gate). stark → reseat (pending).

## 2026-06-14 16:41 — PR #184: λ-robust verify-tree topology (lower the self-KV bar below #83's 0.838) — 🔴 NEGATIVE / BANK-NEGATIVE-ONLY-DEEP-EXTRAP-BEATS-83, topology refuted — MERGED (bank-the-analysis)

- **Branch:** `wirbel/lambda-robust-topology` · **Student:** wirbel · merged 16:41:13Z by morganmcg1 (CPU-only analytic synthesis — no HF Job/submission/served-file change; BASELINE unchanged 481.53, adds 0 TPS; greedy/PPL untouched, no launch authorized). W&B `7uek36mx` (group `lambda-robust-topology`, finished, NaN-clean). Advisor-verified: 7/7 self-test conditions pass.
- **Primary:** `lambda_robust_topology_self_test_passes=1` (7/7). **Test:** `lambda_bar=0.8345` (only an indefensible depth-14 extrapolation beats #83's 0.838 — outside the ≤depth-9 envelope).
- **Key finding:** `BANK-NEGATIVE-ONLY-DEEP-EXTRAP-BEATS-83`. Front-loading acceptance into λ-insensitive shallow depths CANNOT lower the build's λ-bar below #83's 0.838 within any defensible (≤depth-9) topology — only a depth-14 extrapolation reaches `lambda_bar=0.8345`. The min-λ-bar is **identically the max-E[T]@λ=1** (corr −0.95, the same lever #83 already optimized), so **topology alone can NEVER clear 500 at the floor** and never substitutes for self-KV recovery.
- **Conclusion:** HARDENS land #71's measured self-KV λ as the SOLE remaining lever — no topology rewrite buys around the recovery requirement. Closes the topology half of wirbel's mandate (refuted); the N_eff/serial-correlation half carries to the reseat. Pairs with denken #183 (the 0.9052 bar this proves topology can't lower) + land #71 (the build whose λ is now the only knob). wirbel → #190 (N_eff serial-correlation / iid-caveat closure).

## 2026-06-14 16:36 — PR #186: Submission MUST-RETAIN manifest (flag-by-flag packaging de-risk) — 🟢 GREEN / 19 must-retain flags pinned, binding packaging risk = 85.17% of TPS — MERGED (bank-the-analysis)

- **Branch:** `ubel/tree-submission-must-retain-manifest` · **Student:** ubel · merged 16:36:44Z by morganmcg1 (CPU-only manifest consolidation — no HF Job/submission/served-file change; BASELINE unchanged 481.53, adds 0 TPS; greedy/PPL untouched, no launch authorized). W&B `u9kje7sn` (group `tree-submission-must-retain-manifest`, finished, NaN-clean). Advisor-verified: 25/25 self-test conditions pass (independently re-checked manifest flag-values vs the served config — `CENTROID_TOP_K=64`, `num_speculative_tokens=7`, `FUSED_SPARSE_ARGMAX_BLOCK=16` all match).
- **Primary:** `manifest_self_test_passes=1` (25/25). **Test:** `binding_packaging_cost_pct=85.17%`.
- **Key finding:** consolidates the cost-of-omission of every load-bearing serving flag into ONE verified table (n_flags=22, n_must_retain=19, n_double_load_bearing=5); the self-test re-loads the banked JSONs at runtime. The binding packaging risk is **row 1: reverting `relocate_salvaged_kv` to the host-loop = 516→77 TPS (1571× collapse = 85.17% of projected TPS)** — the single flag whose omission destroys the submission. The manifest faithfully carries the pinned stack so the as-submitted build reproduces the served frontier.
- **Conclusion:** the packaging de-risk gate for the eventual launch — every must-retain flag enumerated with its measured cost-of-omission, so land #71's submission build can be diffed against a verified manifest. Pairs with land #71 (the build this protects) + ubel #181 (the composition this packages). ubel → #189 (executable-submission-gate).
