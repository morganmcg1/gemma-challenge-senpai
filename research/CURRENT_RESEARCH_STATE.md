...
- **Date:** 2026-06-15 ~06:12Z (cycle 52p)
...
## Active Roster (cycle 52p, 8/8 GPUs — build-economics matrix: numerator-reachability + private-bar CLOSED; reachability now = a₁-trainability)

| Student | PR | Hypothesis | Owner | Status |
|---------|-----|-----------|-------|--------|
| wirbel  | #307 | EAGLE-3 served-path integration readiness (once a trained drafter exists, is it a CONFIG drop-in to the deployed served runner — speculator config + checkpoint path at launch — or does it need a SERVED-FILE change that re-opens Issue #272's `_guard_included_router` boot-500 risk? LOCAL code-review + dry-run loader audit, no served edit) | Morgan | 🔄 WIP (reseat; #303 CLOSED by Morgan 06:0X — de-conflicted into denken #304 which owns the numerator-reachability inversion; wirbel moved to the orthogonal integration-readiness axis) |
...
| fern    | #305 | EAGLE-3 GO/NO-GO consolidated decision card (deterministic projection at the corrected E[T]=6.11 target + TORNADO ranking each priced axis's banked uncertainty by Δ-TPS + P(clear 500) on public/private; CONDITIONAL on reachability #294/#303/#304 — the rollup the matrix has been building toward) | me | 🔄 WIP (reseat; #302 MERGED 05:51 → the #287 read-cut FITS the build's 0.0428 PPL headroom (cost 0.0203) but REGRESSES on the wall ratio −2.0177 ≈ #273's −2.018, classification `regresses` → BUILD ALONE; read-companion axis (i) CLOSED on both doors) |
| lawine  | #309 | Does the M=8 verify-tree relax denken #304's a₁-cliff-break demand? (#304 demands a₁ 0.73→0.92 at the DRAFT level; lawine #300 showed the deployed M=8 verify-tree ALREADY recovers the position-1 cliff on the LINEAR drafter — does that tree-salvage TRANSFER to a fusion draft, relaxing the a₁ requirement below 0.92? the reconciliation of #300 vs #304) | Morgan | 🔄 WIP (reseat; #300 MERGED 06:02 → build CLEARS private-500 deployed-effective `rho_priv_e3`=0.9421, scalar-0.804 over-prices the private tax 4.8×, axis (g) private-bar CLOSED) |
| denken  | #308 | Is a₁≈0.92 a trainable target or an intrinsic position-1 floor? (denken #304 proved hitting 6.11 REQUIRES lifting a₁ 0.73→0.92 ×1.263; this prices whether a built {2,21,39}-fusion EAGLE-3 drafter can plausibly hit a near-uniform ~0.92 with a BROKEN position-1 cliff, or whether the cliff is a structural floor — the build's central open risk) | Morgan | 🔄 WIP (reseat; #304 MERGED 06:02 → `heavier_step_demands_a1_break=1`, hitting 6.11 DEMANDS a₁ 0.73→0.92, cliff-kept caps at 1+0.73·7=6.11<6.1112 even at b*=1.0, axis (j) numerator-reachability CLOSED) |
| ubel    | #306 | Does the build's VRAM headroom survive ONEGRAPH capture? (resident #299 fits at 20.10 GB / 3.90 GiB headroom, but does the CUDA-graph capture peak + the M-wide 262k-vocab tree-verify logit transient eat it? — lawine #101 size-29 capture-crash precedent; peak-vs-resident refinement) | me | 🔄 WIP (reseat; #299 MERGED 05:51 → build resident 20.10 GB FITS 24GB hard with 3.90 GiB headroom (2.90 vs 23 usable), dominant term extra_kv 0.719 GiB, drafter weights negligible 0.037 GiB; VRAM axis (e) CLOSED at FITS-at-rest) |
...
- **(g) PRIVATE-bar target — CLOSED at CLEARS** — lawine #300 (`8t5q6sr0`, MERGED): the public-E[T] build CLEARS the binding PRIVATE ≥500 gate on the **deployed-effective** model (`rho_priv_e3`=0.9421 > 0.804 → deep-position fidelity closes the gap; raw/no-recovery 0.7797 < 0.804 → does not). The 0.804 is the RAW width-1 spine ratio, but the organizer-verified 460.85 = the **tree-recovered 0.955** (deployed M=8 verify-tree salvages rank-2+, position-1 HELD c₁=1.0, `c_deep`=0.97135 on j≥2) → the per-position private tax is **4.8× smaller** than the scalar-0.804 folklore (private-500 needs public E[T]≈**4.19**, not 4.96). wirbel #290's 4.9029 clears at 579 TPS (deployed-effective) but is NOT robust across the collapse bracket (raw → 479.5, misses); 6.1245/6.11 clears in BOTH (720/589.8). Open reconciliation: does the M=8 tree-salvage TRANSFER to a FUSION draft (the #300-vs-#304 question, reseated lawine #309)?
...
- **(k) RUNTIME VRAM PEAK** — ubel #306 (WIP, NEW): the runtime refinement of (e) — does the #299 resident 3.90 GiB headroom SURVIVE the ONEGRAPH CUDA-graph capture peak + the M-wide 262k-vocab tree-verify logit transient, or does the peak eat the headroom and OOM (lawine #101 size-29 capture-crash precedent)? "Fits at rest" ≠ "fits at runtime."
- **(j) NUMERATOR REACHABILITY — CLOSED (a₁-BREAK DEMANDED)** — denken #304 (`dtf1ouml`, MERGED); wirbel #303 CLOSED by Morgan (de-conflicted into #304): with #295 closing the step-cost DENOMINATOR, the NUMERATOR question — is E[T]≥6.11 reachable by the {2,21,39}-fusion 1-layer drafter? — Invert kanna #289's E[T](a_k) map for the required profile (6.12 needs a_1≈0.92, mean≈0.92, vs deployed a_1=0.7293 / LOW-quartile 0.6550 #297) and ground the achievable ceiling in EAGLE-3's published over-linear uplift applied to deployed mean-accept ≈0.81. **The single most gating axis: if the architecture can't reach 6.12 the honest target is the ~5.0 — every COST axis (a,e,h) is moot until the numerator is shown reachable.** Complements kanna #294 (the cheap GO-PROXY — how to TEST reachability vs whether it's plausible). **Denken #304 RESOLVED it (`heavier_step_demands_a1_break=1`):** #289's per-position spec yields only E[T]≈4.92 (1.19 short of 6.11), and cliff-kept (a₁=0.73 fixed) caps at 1+0.73·7=**6.11 < 6.1112 even at the impossible perfect-tail b*=1.0** → hitting 6.11 DEMANDS breaking the position-1 cliff (a₁→~0.9213 uniform, ×1.263 over deployed 0.7292, ×1.41 over the LOW-quartile 0.6550). The build-spec is **a₁-break + deep-lift**, strictly harder than #289 specified. The open reachability sub-question — is a₁→0.92 TRAINABLE or an intrinsic position-1 floor? — is reseated to **denken #308**; whether the M=8 verify-tree's rank-2+ salvage (lawine #300) RELAXES the draft-level a₁ demand is reseated to **lawine #309**.
...
**ROLLUP (fern #305, WIP, NEW):** with axes (a) step 6.11, (b) companion 0.998, (c) per-position uniform, (e) VRAM 20.10 GB/3.90 GiB, (h) cost 107.47 GPU-hr GO, (i) read-companion CLOSED all banked, fern assembles the consolidated GO/NO-GO **decision card** — the deterministic projection at the corrected E[T]=6.11 target (= the 500 line by construction), a TORNADO ranking each priced axis's banked uncertainty by Δ-TPS (which axis most threatens 500), and P(clear 500) on public/private. CONDITIONAL on the reachability axes — (j) is now CLOSED at **a₁-break DEMANDED** (denken #304), so the residual open reachability is **a₁-TRAINABILITY** (denken #308: is a₁→0.92 trainable or an intrinsic floor?), the **tree-salvage transfer** (lawine #309: does the M=8 verify-tree relax the draft-level a₁ demand?), and **(d)** the cheap viability gate (kanna #294). The card prices "IF a₁→0.92 is reachable, here's the number and the risk," the single artifact the human weighs. Not a launch recommendation.
