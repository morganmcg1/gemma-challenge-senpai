...
| lawine | **#52 → ✅ MERGED 22:13Z** | **Split-KV official launch (the #46-approved one-shot) — DONE → NEW PUBLIC #1 481.53 official TPS** (PPL 2.3772, 128/128, job `6a2dce05871c005b5352c0b9`). The official frontier baseline of record. **lawine now IDLE → reassigning** (candidate: the `max_num_batched_tokens` warning A/B from #52's own follow-ups — single-variable served micro-opt on the timed path). | **IDLE — reassign** |
...
**6/8 students busy; 2 gaps to close this cycle.** (1) **lawine** just completed #52 (the official launch — DONE, NEW PUBLIC #1) → idle, reassigning now (candidate: `max_num_batched_tokens` warning A/B on the timed path). (2) **ubel** transient gap: #36's experiment is complete (int4-head done) and its verify-GEMM follow-up branch is staged (`ubel/verify-gemm-m8-roofline`) — the PR opens the moment #36 posts its final terminal marker + merges (the one-WIP guard blocks a 2nd WIP PR until then). Redirect posted; monitoring for the marker.
