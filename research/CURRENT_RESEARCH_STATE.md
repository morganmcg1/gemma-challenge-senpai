...
- **MILESTONE (2026-06-13 ~13:42Z): first official a10g-small int4 number LANDED VALID.**
  - **PR #3 / stark — int4-qat: ✅ COMPLETE & VALID.** job `6a2d55c7234ca64b60121a6f` (run `results/senpai/int4-qat-20260613T130614Z`): **tps 95.46 · ppl 2.0057 · 128/128 · total_tps 144.53 · duration 686.5s · job_status COMPLETED.** The **2.17× lift over bf16 (44 → 95.46)** and base rung of the reproduction ladder. **Cold-start risk did NOT bite:** `ppl_summary.json` wrote 13:42:23Z, before the ~13:46Z cap. Advisor comment posted requesting the terminal `SENPAI-RESULT` marker + same-path/greedy verdicts + draft→review; **merge-ready once stark posts terminal** (still draft/WIP as of 13:47Z). Two redundant run dirs (`...T131417Z`, `...T131844Z`) ignored; canonical record is `T130614Z`.
  - **PR #4 / lawine — int4-g128-lmhead: ⏳ in flight.** job `6a2d5a96234ca64b60121aa5` launched 13:26:46Z (run `results/senpai/int4-g128-lmhead-20260613T132645Z`), server ready 13:39:26Z (12.6 min cold start), 40-min cap (~14:07Z). `summary.json` not yet written. Expected ~127 TPS / PPL ~2.02 / 128/128. Same cold-start/PPL-cutoff watch (g128+untied-lm_head rebuild ate startup).
...
| stark | #3 (WIP) | **int4 QAT W4A16** reproduction — ✅ **OFFICIAL RUN COMPLETE & VALID** (job `6a2d55c7`): **tps 95.46 / ppl 2.0057 / 128/128**, PPL inside cap. Merge-ready pending stark's terminal `SENPAI-RESULT` marker + same-path/greedy verdicts + draft→review. | **95.46 TPS / PPL 2.0057 ✓** — first official int4 base; merge then assign next rung |
| lawine | #4 (WIP) | **int4 g128 + untied int4 lm_head** re-quant — local PPL 2.0190 ✓, local TPS ~128 ✓, **GREEDY_IDENTICAL 128/128 ✓**; **official run ⏳ IN FLIGHT** (job `6a2d5a96`, server ready 13:39Z, cap ~14:07Z) — `summary.json` not yet written. Rebase onto advisor branch pending (post-job cleanup). | ~127 TPS / PPL ~2.02; review result when it lands, merge if valid |
...
_Last updated: 2026-06-13 (cycle 11 — **first official int4 number LANDED: stark #3 int4-qat 95.46 TPS / PPL 2.0057 / 128/128, VALID, PPL inside the 40-min cap** — merge-ready pending stark's terminal marker; lawine #4 int4-g128 in flight (cap ~14:07Z). Board frontier now pupa-lf29cap444 459.21 / nfs-cap440 457.08 marked VALID at top — but LF29 fold same-path PPL = 2.5499 > cap (frantic-penguin confirmed); cmpatino split one pupa result VALID / another INVALID on the private set, so the verifier catches some but not the prompt_logprobs-keyed bypass. Honest ceiling stays 421 (kenyan-duma valid) → 424.5 (frantic-penguin precache+noscatter pending). pupa tree-shadow quantified the tree-salvage lever at 43.1% sibling miss-rescue. #9 (land) rebased → MERGEABLE; #18 (denken) confirmed fresh not stale (opened 11:44Z today). wirbel #22 same-path gate is the integrity probe + evals-taskforce contribution.)_
