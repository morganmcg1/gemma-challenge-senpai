...
- **Date:** 2026-06-14 ~12:08Z (cycle 47)
...
Tree is the sole 500+ path (tree-free caps 491.8, denken #123 RED). The decisive finding: **descent (BUG-2) alone clears 500** (fern #134 + wirbel #135, two independent methods). land #71 builds the descending accept walk; supported by **lawine #147** (sync-audit harness to verify the sync-free build + measure the real step — #143 MERGED 🟢 closed the denominator's last component: salvage-walk is GPU-hidden, +0.39% sync-free / +2.2% sync-bound, sync-free rule handed to land), **fern #145** (deep-spine width-vs-spread decomposition — the watched 537.8-vs-376 risk, armed for land's readout), and **wirbel #146** (measured-gate confidence envelope — sampling CIs + required-N so the verdict is statistically robust). **BUG-1 depth-1 is now CLOSED-and-handed (denken #133 MERGED):** fp32 GPU-confirmed not-the-fix (lane closed, tree-488-pw-fp32-v0 quota saved); the 13.1pp deficit is ~96% a fixable wrong-rank `target_logits_indices` plumbing bug → handed to land as the rank-1 spine fix, and the **same index-map class may fix BOTH the depth-1 spine AND the descent**. BUG-3 (CUDA-graph illegal-memory-access) lives in the external star-attn build → re-homed to land #71 / build team (ubel #139 closed as out-of-scope for an isolation-scoped seat).

### Theme 3: Launch-Readiness / Projection-Validity Cluster (wirbel #146 + ubel #148)
The memory-BW micro-lever surface is now **fully exhausted** — weights floored at int4 (kanna #132), verify-GEMM weight-BW maxed (#117/#130/#108), scale bytes palette-harvested (#110) with coarser-grouping closed (ubel #140 RED), and the bf16 KV-cache lever closed (wirbel #141 RED). Both freed seats re-home to **bulletproofing the one human-approved 500 shot** — the projected number must survive a skeptical approver. **wirbel #146** = the *sampling* leg (CIs + required-N on land's measured E[T] / branch-hit / step so a GREEN isn't a noisy 1024-step draw). **ubel #148** = the *calibration* leg (does K_cal=125.268, fit on the linear frontier, transfer to the tree path? — the projection's most load-bearing constant, validated independently). With fern #142 (point gate), fern #145 (facet decomp), and lawine #147 (sync-audit + real step), they form the full launch evidence-line.
