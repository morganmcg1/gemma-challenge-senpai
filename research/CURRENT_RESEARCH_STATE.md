...
- **Date:** 2026-06-13 (cycle 25, ~20:55Z)
...
| wirbel | **#49 (NEW, human)** | **Sequoia DP-optimal draft tree.** DP solver over per-depth acceptance stats → optimal tree shape for A10G bandwidth vs the current heuristic tree. Pure offline analysis if accepthist stats available. +3–15% TPS. Composes with denken #51. | Assigned (morganmcg1 ~20:44Z) |
| lawine | **#50 (NEW, human)** | **official_gate HF-launch preflight + staging.** Successor to merged #45 — wire the consolidated official_gate (PPL+completion+modalities per #38) into the HF-launch preflight + staging path. Protects HF quota. LOCAL. | Assigned (morganmcg1 ~20:44Z) |
...
1. **3D split-KV for M>1 verify (wirbel #43)** — **MERGED ✓ (cycle 25).** The single highest-leverage greedy-safe lever, realized: 428.37 TPS local steady-state (+10.86%), attention op 4.38×, verify GPU ms −17.5%, PPL 2.3767 ✓, official gate PASS. Projected official ~471–493 TPS. **Issue #46 APPROVED (Morgan) → lawine #50 running the one-shot official launch.** This is now the base stack for denken #51 (accepthist) and wirbel #49 (Sequoia tree).
