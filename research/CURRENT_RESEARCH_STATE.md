...
| lawine | **#45 (NEW)** | **Local official-gate preflight.** #42 MERGED (`--spec-off` one-flag contract for all 3 spec stacks, proven on-GPU; validator N-mismatch legibility; 14/14 tests). Reassigned: add the missing **modalities-load check** + consolidated `official_gate` verdict (PPL+completion+modalities per #38, separated from the internal greedy bar) to `validate_submission.py`; bundle the canonical fa2sw-reference `--spec-off` regen. Protects HF quota; orthogonal to kanna #44. LOCAL ONLY. | **Assigned (post-#42 merge)** |
| fern | **#34 (WIP)** | **Benchmark-matched reasoning corpus → EAGLE-3 retrain.** Self-distill greedy CoT from served target under EXACT benchmark prompt templates (MCQ `ANSWER: $LETTER` for mmlu_pro/gpqa, step-by-step `ANSWER: $ANSWER` for aime) on MMLU-Pro/GPQA/AIME (57/57/14), hard-dedup vs 128 eval ids, early-stop on held-out reasoning tf_acc. Target: break 0.73 plateau toward 0.78–0.85. | Active — **native cross-check + step-0 convention check** (land #9 measured the off-by-one: verify EAGLE-3 export feeds `f_{j-1}` not `f_j`, else gains won't convert to native) |
| denken | **#41 (WIP)** | **Scatter floor elimination in `compute_logits`.** Step 1 equivalence proof **VERIFIED** (`equiv_rate=1.0`, proven *universal* — ascending `kept_ids` ⟹ generalizes to private set) + microbench **VERIFIED** (scatter 0.348 / persistent 0.299 ms @ M=45; deployable bit-identical persistent buffer). **Sent back: Step-4 ceiling W&B mismatch** — cited runs log K=6→480/477 (`>500=False`), not claimed K=11→544/540; 538.15 control absent; `primary_metric=544.22` unsubstantiated. Re-run tree ceiling at K=11/M=45 + link, or correct table+primary_metric. | Active — request-changes (reconcile W&B) |
...
| land | **#9 (WIP)** | **Wide KL-distilled drafter** — rebased CLEAN (`67a2bc7`). **Root cause MEASURED:** serve feeds step-0 `f_{j-1}` (EAGLE convention), v0/v1b trained `f_j` → L2≈3 vs 200–355; single off-by-one = entire tf↔native gap. **v2 in flight** (`--serve-faithful-hidden` step-0=`f_{j-1}` + `--free-run-prob 0.0` HASS-canonical; W&B `land-hass-v2-192212`; select on `heldout_native_accept_per_step` @ 50% ckpt). Primary: native > stock 3.553. | Active — v2 training (endorsed) |
...
6. **Official-gate preflight (lawine #45)** — #42 MERGED (`--spec-off` one-flag contract). Next: reproduce the *official* gate locally (PPL+completion+**modalities**, per #38) with a consolidated `official_gate` verdict, separated from the over-strict internal greedy bar. Adds the one official criterion we don't currently check (modalities-load) → catches skip-load/zero-cap stacks before HF launch. Pairs with kanna #44 (private-gap) as the pre-submission de-risking front-door.
