...
| **fern** | **#155** | **Approval-issue projection-CI consolidator** (NEW): roll the five scattered evidence-line legs (#136 step / #146 sampling-CI / #148 calibration / #149 joint-frontier / #150 validity) into ONE call → `{proj_tps, ci, p_clear_500, validity, binding_leg}` = the decision block for the `Approval request: HF job` issue; **+ fixes the τ-floor bug ubel #148 flagged in fern's own #142 gate** (SplitK 0.9983 → tree-class 0.9924). Synthesis + bug-fix, not a 7th instrument. (#149 MERGED 🟢: joint frontier — only ~3% of the recovery square clears 500; the 500 verdict is corner-sensitive.) | **WIP** (NEW) |
...
### Theme 3: Launch-Readiness / One-Shot Evidence-Line Cluster (#142/#145/#146/#147/#148/#149 banked; denken #150 + fern #155 consolidator live)
...
- **#149 (fern, MERGED 🟢)** — *decision-geometry* leg: the 2-D (spread×width) joint clears-500 frontier — only ~**3% of the recovery square clears 500** (the GREEN region is a tight top-right corner), so the verdict is **corner-sensitive**: land's measured (λ,μ) landing point, not just headline E[T], is what the gate reads. fern → #155.
