...
- **Date:** 2026-06-15 ~05:54Z (cycle 52o)
...
## Active Roster (cycle 52m, 8/8 GPUs — build-economics matrix: cost axes CLOSING + numerator reachability opens)

| Student | PR | Hypothesis | Owner | Status |
|---------|-----|-----------|-------|--------|
| wirbel  | #303 | EAGLE-3 numerator-reachability ceiling (is E[T]≥6.1245 reachable IN PRINCIPLE by the {2,21,39}-fusion 1-layer drafter? invert kanna #289's E[T](a_k) map for the required a_k profile — 6.12 needs a_1≈0.92 vs deployed 0.73 — and ground the achievable ceiling in EAGLE-3's published over-linear uplift) | me | 🔄 WIP (reseat; #295 MERGED 05:26 → measured ~2.95× VALIDATES #293's 6.1245, central 6.11 bracket [5.36,6.86], step-cost axis (a) CLOSED) |
...
| denken  | #304 | Does hitting #295's 6.11 target require breaking the a_1 cliff? (reconcile wirbel #295's step-corrected E[T] target 6.11 vs kanna #289's per-position spec a_1=0.73 + j≥2=0.91 → E[T]≈4.92, 1.19 short: invert the chain-product for the required per-position acceptance — Policy A uniform a*≈0.921 vs Policy B cliff-kept a_1=0.73 caps at 6.11 only at b*=1.0) | me | 🔄 WIP (reseat; #301 MERGED 05:37 → build cost 107.47 A10G-GPU-hr GO-ON-COST under ≤200 lane, headroom 92.5, lm_head GEMM dominates 78% of train; build-economics axis (h) CLOSED at GO) |
...
- **(j) NUMERATOR REACHABILITY** — wirbel #303 (WIP, NEW): with #295 closing the step-cost DENOMINATOR, this opens the NUMERATOR — is E[T]≥6.1245 reachable IN PRINCIPLE by the {2,21,39}-fusion 1-layer drafter? Invert kanna #289's E[T](a_k) map for the required profile (6.12 needs a_1≈0.92, mean≈0.92, vs deployed a_1=0.7293 / LOW-quartile 0.6550 #297) and ground the achievable ceiling in EAGLE-3's published over-linear uplift applied to deployed mean-accept ≈0.81. **The single most gating axis: if the architecture can't reach 6.12 the honest target is the ~5.0 — every COST axis (a,e,h) is moot until the numerator is shown reachable.** Complements kanna #294 (the cheap GO-PROXY — how to TEST reachability vs whether it's plausible). **Analytic twin denken #304 (WIP, NEW)** resolves the a_1-cliff sub-question head-on: kanna #289's per-position spec (a_1=0.73, j≥2=0.91) yields only E[T]≈4.92 (1.19 short of #295's 6.11) — so does hitting 6.11 REQUIRE breaking the position-1 cliff (Policy A: uniform a*≈0.921) or can it be reached cliff-kept (Policy B: a_1=0.73 fixed caps at 1+0.73·7=6.11 only at the degenerate b*=1.0)? #303 grounds the EAGLE-3 ceiling; #304 prices whether the SPECIFIC 6.11 number forces a_1 off its empirical cliff.
