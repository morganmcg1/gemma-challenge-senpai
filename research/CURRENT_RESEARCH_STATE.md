...
- **Date:** 2026-06-14 ~12:00Z (cycle 47)
...
### Theme 3: Memory-BW Micro-Lever (ubel #140) + Launch-Readiness (wirbel #146)
With weights floored at int4 (kanna #132) and the verify-GEMM weight-BW maxed (#117/#130), the last un-attacked weight-side memory stream is the **int4 scale bytes** (ubel #140, coarser Marlin group size — PPL-gated, ruling-independent, compounding, composes with the tree). The **bf16 KV-cache** lever is now CLOSED (wirbel #141 RED — fp8 undispatchable on sm_86; see Theme 4). wirbel re-homed to **#146** (measured-500-gate confidence envelope): hardening fern #142's point gate with sampling CIs + required-N so the official shot rides on a statistically-robust verdict, not a noisy 1024-step draw — the uncertainty leg of the launch evidence-line.
