...
- **Date:** 2026-06-15 ~06:05Z (cycle 52p)
...
## Active Roster (cycle 52p, 8/8 GPUs — build-economics matrix NEAR-FULLY CLOSED; frontier reframes to drafter TRAINABILITY + deployment integration)

| Student | PR | Hypothesis | Owner | Status |
|---------|-----|-----------|-------|--------|
| wirbel  | #307 | EAGLE-3 served-path integration-readiness (is wiring the {2,21,39}-fusion drafter into the deployed served runner a CONFIG drop-in, or a served-file/fork change that re-opens the Issue #272 boot-500 risk? READ-ONLY static analysis → `swap_is_config_only` / `reuses_boot_fragile_path` / `readiness_blocks_go`) | me | 🔄 WIP (reseat after #303 de-conflict→closed; the unpriced DEPLOYMENT-feasibility axis) |
| kanna   | #294 | EAGLE-3 Phase-1 viability gate (the cheap-proxy GO threshold before the human-gated retrain) | Morgan | 🔄 WIP |
| fern    | #305 | EAGLE-3 GO/NO-GO consolidated decision card (deterministic projection at E[T]=6.11 + TORNADO Δ-TPS ranking + P(clear 500) public/private; CONDITIONAL on reachability/trainability) | Morgan | 🔄 WIP |
| lawine  | #309 | Does the M=8 verify-tree relax denken #304's a1-cliff-break demand? (VERIFY-side: #300 showed the tree recovers c1=1.0 on the LINEAR drafter — does that salvage transfer to a fusion draft, lowering the required RAW a1 below 0.92?) | me | 🔄 WIP (reseat; #300 MERGED 06:02 → private bar CLEARS, ρ_priv 0.9421, c1=1.0 tree-recovered, axis (g) CLOSED) |
| denken  | #308 | EAGLE-3 a1-cliff trainability (is a1≈0.92 inside the published EAGLE-3 first-token envelope or an INTRINSIC position-1 floor? DRAFT-side complement to lawine #309) | me | 🔄 WIP (reseat; #304 MERGED 06:02 → heavier step DEMANDS a1: 0.73→0.92, E[T]=4.92 falls 1.19 short of 6.11, axis (j) CLOSED) |
| ubel    | #306 | Does the build's VRAM headroom survive ONEGRAPH capture? (runtime PEAK vs #299's 20.10 GB resident / 3.90 GiB headroom; size-29 capture-crash precedent) | Morgan | 🔄 WIP |
...
**ROLLUP (fern #305, WIP):** with axes (a) step 6.11, (b) companion 0.998, (c) per-position uniform, (e) VRAM 20.10 GB/3.90 GiB, (g) private CLEARS (ρ_priv 0.9421), (h) cost 107.47 GPU-hr GO, (i) read-companion CLOSED, AND (j) numerator-reachability CLOSED (the 6.12 needs a_1→0.92, denken #304) all banked, fern assembles the consolidated GO/NO-GO **decision card** — the deterministic projection at the corrected E[T]=6.11 target (= the 500 line by construction), a TORNADO ranking each priced axis's banked uncertainty by Δ-TPS (which axis most threatens 500), and P(clear 500) on public/private. The matrix is now NEAR-FULLY CLOSED; the live residual is the **TRAINABILITY** of the a_1-cliff-break drafter that (j) showed is required — (d) kanna #294 Phase-1 gate + the a_1-trainability twins denken #308 (draft-side) / lawine #309 (verify-side salvage). It prices "IF the required a_1≈0.92 drafter is trainable, here's the number and the risk," the single artifact the human weighs. Not a launch recommendation.
