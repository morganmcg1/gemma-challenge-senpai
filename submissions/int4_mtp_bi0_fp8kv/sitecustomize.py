...
# Relax vLLM's fp8_e5m2 KV guard for weight-only compressed-tensors checkpoints
# (no kv_cache_scheme). e5m2 is the only fp8 KV dtype the Triton compiler can emit
# on Ampere/sm_86 (e4m3 -> fp8e4nv is unsupported there), so this is what lets the
# fp8 KV cache boot at all on A10G while keeping the fully-compiled decode path.
# Strict no-op for non-e5m2 dtypes and for genuine fp8-KV checkpoints.
_install_hook(
    "vllm.model_executor.layers.attention.attention",
    _make_applier("vllm_fp8kv_e5m2_guard_patch", "fp8_e5m2 KV guard relaxation"),
)
