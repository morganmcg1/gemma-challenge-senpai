...
# byteexact-splitkv (lawine PR #496 lever, packaged for #500): pin the 3D split-KV
# tiles_per_segment to a fixed constant so segment boundaries fall at fixed ABSOLUTE
# key positions -> the M=8 spec-verify reduction is byte-identical to M=1 AR decode
# (fixed split SIZE, not count). Env-gated; BYTEEXACT_FIXED_TPS>0 (manifest sets 4)
# and BYTEEXACT_NUM_SEGMENTS (manifest sets 64). Re-jits both @triton.jit kernels and
# raises triton_attn.NUM_PAR_SOFTMAX_SEGMENTS. Imported AFTER splitkv so its meta-path
# finders compose on top (splitkv routes verify -> 3D; this makes 3D byte-exact). Does
# NOT set VLLM_BATCH_INVARIANT, so the MLP/QKV/lm_head Marlin matmuls keep the fast
# path. Off by default -> served compute path byte-identical to the parent
# fa2sw_precache_kenyan stack. See module docstring.
if int(__import__("os").environ.get("BYTEEXACT_FIXED_TPS", "0") or "0") > 0:
    import byteexact_splitkv_patch  # noqa: E402,F401
