...
def main() -> None:
    # int4 QAT W4A16 checkpoint. vLLM auto-detects the compressed-tensors
    # quantization_config (pack-quantized, num_bits=4, group_size=32) from the
    # model config and selects the Marlin int4 W4A16 kernel; no --quantization
    # flag is needed. The checkpoint's quant `ignore` list keeps the vision and
    # audio towers in bf16, so all modalities load. The harness overrides this
    # default via the MODEL_ID env it derives from manifest.json "model_id".
