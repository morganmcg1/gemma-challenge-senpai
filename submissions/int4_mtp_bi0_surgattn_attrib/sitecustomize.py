...
We install three runtime patches, each applied the moment its target vLLM module
is first imported:
...
* ``vllm_force2d_attn_patch`` on ``vllm.v1.attention.backends.triton_attn`` -- forces
  the 2D single-pass attention path, gated on forward type (drafter-proposer vs
  main-model) so this attribution card can recover the byte-identical drafter share
  of the surgattn 3D speedup. With ``SURGATTN_ARM`` unset it is the shipped bi0
  behaviour: 2D forced on every M=1 forward.
* ``vllm_proposer_ctx_patch`` on ``vllm.v1.spec_decode.llm_base_proposer`` -- marks
  the speculative-proposer scope (``propose`` / ``dummy_run``) so the force-2D
  wrapper can tell a drafter forward from a main-model forward (no-op without
  speculation).
...
_install_hook(
    ...
    _make_applier("vllm_proposer_ctx_patch", "proposer scope marker patch"),
)
