...
SURGATTN-3D STACK VARIANT (PR #797): this is a one-delta copy of
``int4_mtp_bi0_int4head``. The ONLY change is that the ``vllm_force2d_attn_patch``
install hook below is intentionally NOT registered, so the native TRITON_ATTN 3D
split-KV (flash-decoding) path runs on the M=1 decode forward instead of bi0's
shipped force-2D single-pass path. Everything else (int4 g32 lm_head via model_id,
the attention-group num_heads patch, the prometheus route-name guard, the MTP
drafter at K=6, BI=0) is byte-identical to the CONTROL submission. Dropping force-2D
gives up byte-exact greedy identity (surgattn-3D is a #784-band candidate: PPL-gated,
identity optional) in exchange for the native 3D split-KV decode-kernel speedup.

We install one runtime patch (and intentionally skip a second), each applied the
moment its target vLLM module is first imported:
...
* ``vllm_force2d_attn_patch`` on ``vllm.v1.attention.backends.triton_attn`` -- DISABLED
  in this STACK variant (see above). When registered (CONTROL submission) it forces
  the 2D single-pass attention path for decode and spec-verify, recovering byte-exact
  greedy identity; leaving it off lets the native 3D split-KV path run.
...
# SURGATTN-3D STACK VARIANT (PR #797): the force-2D install hook is intentionally
# omitted here so the native TRITON_ATTN 3D split-KV path runs on M=1 decode. The
# CONTROL submission (int4_mtp_bi0_int4head) registers it:
#     _install_hook(
#         "vllm.v1.attention.backends.triton_attn",
#         _make_applier("vllm_force2d_attn_patch", "force-2D attention patch"),
#     )
# With it omitted, serve.log will NOT print the
# "[int4_mtp_force2d] unified_attention wrapped: forcing 2D single-pass" line.
# vllm_force2d_attn_patch.py is left in the package (unimported) so the only
# behavioral diff vs CONTROL is this single removed registration.
