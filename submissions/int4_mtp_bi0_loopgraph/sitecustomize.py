...
def _make_applier(patch_module_name, label, func="apply"):
    """Build an applier that imports ``patch_module_name`` and calls its
    ``func(module)`` against the freshly-imported vLLM target module, logging
    (but not raising) on failure so a patch bug can never wedge the server.

    ``func`` selects which entry point to call so one patch module can expose
    several appliers (the loopgraph module patches both the Gemma4 proposer and
    the GPUModelRunner draft-token copy path)."""
...
# --- bi0-loopgraph experiment patches (PR #771) ---
# Port of the fa2sw loopgraph (onegraph) CUDA-graph draft replay + fused Triton
# sparse-argmax onto plain int4 bi0, on stock vllm==0.22.0. Both are drafter-only
# / argmax-preserving: they change HOW the K-step draft loop is dispatched and HOW
# the centroid top-token is computed, never the verifier's emitted token at T=0, so
# greedy identity / PPL are structurally unchanged; only acceptance rate / TPS can
# move. gpu_model_runner is patched by BOTH the attn-group hook above and the
# loopgraph copy-event hook below; the one-shot _PatchFinder chains them (each
# removes itself before re-resolving the spec), so both appliers run.
_install_hook(
    "vllm.v1.spec_decode.gemma4",
    _make_applier("vllm_loopgraph_patch", "loopgraph proposer patch", "apply_proposer"),
)
_install_hook(
    "vllm.v1.worker.gpu_model_runner",
    _make_applier(
        "vllm_loopgraph_patch", "loopgraph draft-token copy-event patch", "apply_copy_event"
    ),
)
_install_hook(
    "vllm.model_executor.models.gemma4_mtp",
    _make_applier("vllm_fused_argmax_patch", "fused sparse-argmax patch"),
)
