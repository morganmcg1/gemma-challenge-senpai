...
Two deltas vs the official `...-qat-w4a16-ct` checkpoint:
  ...
  2. Untie `lm_head` (== embed_tokens, 262144x2560) and quantize it to int4 too,
     with a CHANNEL-WISE (group_size=-1, one scale per output channel) scheme —
     the only delta vs the g128-head sibling submission. `embed_tokens` stays bf16.
...
def main() -> None:
    ...
    ap.add_argument("--out", default="/workspace/gemma_build/int4_channel_lmhead")
    ap.add_argument("--module-list",
                    default=str(Path(__file__).parent / "official_quantized_modules.json"))
    ap.add_argument("--group-size", type=int, default=128)
    ap.add_argument("--head-group-size", type=int, default=-1,
                    help="-1 for channel-wise head (this submission), 128 for g128 head")
    ...
    print("[write] wrote config.json (tie_word_embeddings=false, body g128 + channel-wise lm_head)")
