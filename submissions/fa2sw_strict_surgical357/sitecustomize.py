...
# surgical-attn (lawine PR #488 lever, productionized): force the byte-exact 2D
# order-preserving attention path on the full-attention reductions WITHOUT the global
# VLLM_BATCH_INVARIANT matmul tax. Env-gated; SURGICAL_ATTN_USE_3D_OFF=1. Sets
# triton_unified_attention.is_batch_invariant=True (which also short-circuits the
# splitkv-verify 3D redirect, so the M=8 verify takes the same 2D path). Imported AFTER
# splitkv so its meta-path finder composes on top. Off by default -> served compute path
# byte-identical to the parent fa2sw_precache_kenyan stack. See module docstring.
if __import__("os").environ.get("SURGICAL_ATTN_USE_3D_OFF", "0") == "1":
    import surgical_attn_patch  # noqa: E402,F401
